...
function queueEmail(data) {
    if (data.type === "order_placed") {
        Promise.all([
            (0, mailer_1.sendBuyerConfirmation)(data.order),
            (0, mailer_1.sendAdminNotification)(data.order),
        ]).catch((err) => logger_1.default.error(`Email failed [order_placed ${data.order.orderNumber}]: ${err.message}`));
    }
