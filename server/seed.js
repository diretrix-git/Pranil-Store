 ...
 *   - 1 Admin     →  admin@markethub.com / Admin@1234
...
async function upsertUser(data) {
  ...
  if (existing) { console.log(`   ⚠️  ${data.role} already exists (${data.email})`); return existing; }
...
async function seed() {
  ...
  for (const cat of CATEGORIES) {
    ...
    if (existing) { categoryMap[cat.slug] = existing._id; console.log(`   ⚠️  ${cat.name} exists`); }
    else { const c = await Category.create(cat); categoryMap[cat.slug] = c._id; console.log(`   ✅ ${cat.icon} ${cat.name}`); }
  }
  ...
  if (existingCount > 0) {
    ...
    for (const p of PRODUCTS) {
      ...
      await Product.create({ ...rest, categories: categoryIds, category: primaryCat?.name ?? '' });
      console.log(`   ✅ ${p.name}`);
    }
  }
