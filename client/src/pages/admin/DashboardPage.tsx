...
function StatCard({ icon: Icon, label, value, sub, color, bg, border }: { icon: LucideIcon; label: string; value: string | number; sub?: string; color: string; bg: string; border?: string }) {
  return (
    <div className={`bg-white rounded-xl border ${border ?? "border-slate-200"} p-5 shadow-sm`}>
      <div className="flex items-start justify-between mb-3">
        <div className={`p-2 rounded-lg ${bg}`}><Icon size={18} className={color} strokeWidth={2} /></div>
      </div>
...
function QuickLink({ to, icon: Icon, label, desc }: { to: string; icon: LucideIcon; label: string; desc: string }) {
  return (
    <Link to={to} className="flex items-center gap-3 p-4 bg-white rounded-xl border border-slate-200 hover:border-violet-200 hover:shadow-sm transition-all group">
      <div className="p-2 rounded-lg bg-slate-50 group-hover:bg-violet-50 transition-colors"><Icon size={16} className="text-slate-500 group-hover:text-violet-600 transition-colors" /></div>
      <div>
  ...
  return (
    <div className="p-6 max-w-6xl mx-auto">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
        <StatCard icon={AlertTriangle} label="Low Stock" value={s.lowStockProducts ?? 0} sub="Products below 20 units" color="text-red-600" bg="bg-red-50" border="border-red-100" />
        <StatCard icon={Clock} label="Pending Orders" value={s.pendingOrders ?? 0} sub="Awaiting fulfillment" color="text-violet-600" bg="bg-violet-50" border="border-violet-100" />
        <StatCard icon={Truck} label="Active Vendors" value={s.totalVendors ?? 0} sub="Supplier partners" color="text-blue-600" bg="bg-blue-50" border="border-blue-100" />
        <StatCard icon={TrendingUp} label="Revenue" value={formatRs(s.totalRevenue ?? 0)} sub="Total fulfilled volume" color="text-emerald-600" bg="bg-emerald-50" border="border-emerald-100" />
      </div>
