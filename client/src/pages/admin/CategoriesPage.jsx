...
export default function AdminCategoriesPage() {
  ...
  const openCreate = () => { setForm({ name: '', icon: '📦', description: '' }); setFormError(''); setShowModal(true); };
  ...
  return (
    <div className="min-h-screen bg-slate-50">
      ...
      <div className="max-w-4xl mx-auto px-4 py-8 sm:py-10">
        <div className="flex justify-between items-center mb-6">
          <motion.h1 initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }}
            className="text-2xl sm:text-3xl font-black text-slate-900">Categories</motion.h1>
        ...
        {isLoading ? (
          ...
          <motion.div variants={{ show: { transition: { staggerChildren: 0.06 } } }} initial="hidden" animate="show"
            ...
            {categories.map((cat) => (
              <motion.div key={cat._id} variants={{ hidden: { opacity: 0, y: 16 }, show: { opacity: 1, y: 0 } }}
      ...
      <Modal open={!!deleteId} onClose={() => setDeleteId(null)} title="Delete Category?">
        <p className="text-slate-500 text-sm mb-6">Products in this category will not be deleted, but they will lose this category tag.</p>
