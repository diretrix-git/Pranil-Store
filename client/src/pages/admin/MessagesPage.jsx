      ...
      <div className="p-6 max-w-4xl mx-auto">
        ...
        {isLoading ? (
          ...
          <div className="text-center py-20 bg-white rounded-xl border border-slate-200">
            ...
            <p className="text-slate-500">{dateFilter === 'all' ? 'No messages yet.' : 'No messages in this period.'}</p>
          </div>
        ) : (
            ...
            {messages.map((msg) => (
              <button key={msg._id} onClick={() => openMessage(msg)}
                className={`w-full text-left bg-white rounded-xl border p-4 shadow-sm hover:shadow-md transition-all ${
                  !msg.isRead ? 'border-violet-300' : 'border-slate-200'
                }`}>
                <div className="flex items-start justify-between gap-3">
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2 flex-wrap mb-1">
                      ...
                      {!msg.isRead && <span className="bg-violet-100 text-violet-700 text-xs font-bold px-2 py-0.5 rounded-full">New</span>}
                    </div>
            ...
            <motion.div initial={{ opacity: 0, scale: 0.95, y: 20 }} animate={{ opacity: 1, scale: 1, y: 0 }}
              ...
              <div className="flex items-start justify-between mb-5">
                <div className="flex-1 min-w-0 pr-4">
                  ...
                  <p className="text-xs text-slate-400 mt-1">{new Date(selectedMsg.createdAt).toLocaleString()}</p>
                </div>
                <button onClick={() => setSelectedMsg(null)}
                  className="shrink-0 text-slate-400 hover:text-slate-600 w-8 h-8 flex items-center justify-center rounded-lg hover:bg-slate-100 font-bold">✕</button>
              </div>
