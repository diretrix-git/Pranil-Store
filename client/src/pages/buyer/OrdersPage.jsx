    ...
    <div className="min-h-screen bg-slate-50">
      ...
      <div className="max-w-5xl mx-auto px-4 py-8 sm:py-10">
        <motion.h1 initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }}
          className="text-2xl sm:text-3xl font-black text-slate-900 mb-6">My Orders</motion.h1>
              ...
              <div key={order._id}
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-sm text-slate-400">
                    {order.storeSnapshot?.name ?? '—'} · {new Date(order.createdAt).toLocaleDateString()}
                  </p>
                </div>
                <div className="flex items-center gap-4">
                  <span className="text-lg font-black text-slate-900">${Number(order.totalAmount).toFixed(2)}</span>
