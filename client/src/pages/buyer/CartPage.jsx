...
export default function CartPage() {
  ...
  // Local qty state to allow typing without firing API on every keystroke
  ...
  const placeOrder = useMutation({
    ...
    onSuccess: () => {
      ...
      setNotes('');
      // Show success toast — no auto-print, no redirect
  ...
  const handleQtyChange = useCallback((pid, val) => {
    setLocalQty((prev) => ({ ...prev, [String(pid)]: val }));
  }, []);

  const commitQty = useCallback((pid, val) => {
  ...
  const getQty = (pid, fallback) => localQty[String(pid)] !== undefined ? localQty[String(pid)] : fallback;
        ...
        <AnimatePresence>
          {successMsg && (
            <motion.div initial={{ opacity: 0, y: -12 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -12 }}
              className="bg-green-50 border border-green-200 text-green-700 text-sm rounded-xl px-4 py-4 mb-5 flex items-start gap-3">
              <span className="text-lg shrink-0">✅</span>
              <div>
                <p className="font-semibold">{successMsg}</p>
                <p className="text-xs text-green-600 mt-0.5">Check <a href="/orders" className="underline font-semibold">My Orders</a> to view your order details and invoice.</p>
              </div>
            </motion.div>
          )}
        </AnimatePresence>
                        ...
                        <motion.tr key={String(pid)} exit={{ opacity: 0, x: -20 }}
                          ...
                          <td className="px-4 py-4">
                            <input
                              ...
                              onBlur={(e) => commitQty(pid, e.target.value)}
                              onKeyDown={(e) => e.key === 'Enter' && commitQty(pid, qty)}
                              className="w-20 bg-slate-50 border border-slate-200 rounded-lg px-2 py-1.5 text-center text-sm focus:outline-none focus:ring-2 focus:ring-violet-400 focus:bg-white transition-all"
                            />
                          </td>
                ...
                return (
                  <div key={String(pid)} className="bg-white rounded-2xl border border-slate-200 p-4 shadow-sm">
                    ...
                    <div className="flex items-center justify-between">
                      ...
                      <p className="font-bold text-slate-900">{formatRs(Number(item.price) * item.quantity)}</p>
                    </div>
                  </div>
                );
              })}
            ...
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
              ...
              <div className="text-right w-full sm:w-auto">
                <p className="text-xl sm:text-2xl font-black text-slate-900 mb-3">
                  Total: <span className="text-violet-600">{formatRs(total)}</span>
                </p>
                ...
                {items.map((item) => {
                  ...
                  return (
                    <div key={String(pid)} className="flex justify-between text-sm">
                      ...
                      <span className="font-semibold text-slate-900">{formatRs(Number(item.price) * item.quantity)}</span>
                    </div>
                  );
                })}
                <div className="border-t border-slate-200 pt-2 mt-2 flex justify-between font-black text-slate-900">
                  <span>Total</span>
                  <span className="text-violet-600">{formatRs(total)}</span>
                </div>
              </div>
              <div className="mb-5">
