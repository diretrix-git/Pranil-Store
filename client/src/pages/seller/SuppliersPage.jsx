...
const EMPTY = { name: '', contactPerson: '', email: '', phone: '', address: '', categories: '', notes: '' };
...
export default function SuppliersPage() {
  ...
  const openCreate = () => { setEditSupplier(null); setForm(EMPTY); setFormError(''); setShowModal(true); };
  const openEdit = (s) => {
    setEditSupplier(s);
    setForm({ name: s.name ?? '', contactPerson: s.contactPerson ?? '', email: s.email ?? '', phone: s.phone ?? '', address: s.address ?? '', categories: Array.isArray(s.categories) ? s.categories.join(', ') : '', notes: s.notes ?? '' });
    setFormError(''); setShowModal(true);
  };

  const saveMutation = useMutation({
    mutationFn: async () => {
      const payload = { ...form, categories: form.categories ? form.categories.split(',').map((c) => c.trim()).filter(Boolean) : [] };
      return editSupplier ? api.put(`/suppliers/${editSupplier._id}`, payload) : api.post('/suppliers', payload);
    },
  ...
  const formFields = [
    { name: 'name', label: 'Name' }, { name: 'contactPerson', label: 'Contact Person' },
    { name: 'email', label: 'Email', type: 'email' }, { name: 'phone', label: 'Phone' },
    { name: 'address', label: 'Address' }, { name: 'categories', label: 'Categories (comma-separated)' },
    ...
    <div className="min-h-screen bg-slate-50">
      ...
      <div className="max-w-6xl mx-auto px-4 py-8 sm:py-10">
        <div className="flex justify-between items-center mb-6">
          <motion.h1 initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} className="text-2xl sm:text-3xl font-black text-slate-900">Suppliers</motion.h1>
        ...
        {isLoading ? (
          ...
          <div className="text-center py-20"><p className="text-5xl mb-4">🏭</p><p className="text-slate-400">No suppliers yet.</p></div>
        ) : (
          <div className="bg-white rounded-2xl border border-slate-200 overflow-hidden shadow-sm">
            <div className="overflow-x-auto">
              <table className="w-full text-sm min-w-[500px]">
                <thead className="bg-slate-50 border-b border-slate-200">
                  <tr>{['Name', 'Contact', 'Email', 'Phone', ''].map((h) => (
                  ...
                  {suppliers.map((s) => (
                    <tr key={s._id} className="border-b border-slate-100 last:border-0 hover:bg-slate-50 transition-colors">
                      ...
                      <td className="px-4 py-3 text-right space-x-3">
                        <button onClick={() => openEdit(s)} className="text-violet-600 hover:text-violet-800 text-xs font-semibold">Edit</button>
      ...
      <Modal open={showModal} onClose={() => setShowModal(false)} title={editSupplier ? 'Edit Supplier' : 'New Supplier'}>
