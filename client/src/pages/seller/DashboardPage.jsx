...
export default function SellerDashboardPage() {
  const { data: ordersData } = useQuery({
    ...
    queryFn: async () => { const res = await api.get('/orders/store'); return res.data; },
  });
    ...
    <div className="min-h-screen bg-slate-50">
      ...
      <div className="max-w-6xl mx-auto px-4 py-8 sm:py-10">
        ...
        <motion.div variants={{ show: { transition: { staggerChildren: 0.1 } } }} initial="hidden" animate="show"
          ...
          <StatCard icon="📦" label="Total Products" value={products.length} color="text-violet-700" bg="bg-violet-50" delay={0} />
          <StatCard icon="🛒" label="Total Orders" value={orders.length} color="text-indigo-700" bg="bg-indigo-50" delay={0.1} />
          <StatCard icon="💰" label="Total Revenue" value={`$${revenue.toFixed(2)}`} color="text-emerald-700" bg="bg-emerald-50" delay={0.2} />
        </motion.div>

        <motion.div variants={{ show: { transition: { staggerChildren: 0.08 } } }} initial="hidden" animate="show"
          className="grid grid-cols-2 sm:grid-cols-4 gap-4">
          {[
            { to: '/seller/products', icon: '📦', label: 'Products', desc: 'Manage inventory' },
            { to: '/seller/orders', icon: '🛒', label: 'Orders', desc: 'View & fulfill orders' },
            { to: '/seller/suppliers', icon: '🏭', label: 'Suppliers', desc: 'Manage suppliers' },
            { to: '/seller/settings', icon: '⚙️', label: 'Store Settings', desc: 'Edit store info' },
          ].map(({ to, icon, label, desc }) => (
            <motion.div key={to} variants={fadeUp} whileHover={{ y: -3 }}>
              <Link to={to} className="block bg-white border border-slate-200 rounded-2xl p-4 sm:p-5 hover:shadow-md hover:border-violet-200 transition-all group">
                <span className="text-2xl sm:text-3xl block mb-2">{icon}</span>
                <p className="font-bold text-slate-800 group-hover:text-violet-600 transition-colors text-sm sm:text-base">{label}</p>
