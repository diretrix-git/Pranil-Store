...
export default function SellerProductsPage() {
  ...
  const { data, isLoading } = useQuery({
    queryKey: ['seller-products', user?.store],
    queryFn: async () => {
      const storeId = user?.store?._id ?? user?.store;
      const res = await api.get('/products', { params: { storeId } });
  ...
  const openEdit = (p) => {
    ...
    const existingCatIds = (p.categories ?? []).map((c) => c._id ?? c);
  ...
  const saveMutation = useMutation({
    mutationFn: async () => {
      ...
      // Send categories as comma-separated IDs
      fd.append('categories', selectedCategories.join(','));
      imageFiles.forEach((f) => fd.append('images', f));
      return editProduct
        ? api.put(`/products/${editProduct._id}`, fd)
  ...
  return (
    <div className="min-h-screen bg-slate-50">
        ...
        {isLoading ? (
          ...
          <div className="text-center py-20">
            ...
            <button onClick={openCreate}
              className="px-5 py-2.5 rounded-xl text-sm font-bold text-white bg-gradient-to-r from-violet-600 to-indigo-600">
            ...
            <div className="overflow-x-auto">
              <table className="w-full text-sm min-w-[600px]">
                <thead className="bg-slate-50 border-b border-slate-200">
                  <tr>
                    {['Name', 'Categories', 'Price', 'Stock', 'Status', ''].map((h) => (
      ...
      <Modal open={showModal} onClose={() => setShowModal(false)} title={editProduct ? 'Edit Product' : 'New Product'}>
        {formError && (
          <p className="text-red-500 text-sm mb-4 bg-red-50 border border-red-200 rounded-xl px-3 py-2">{formError}</p>
        )}
        <div className="space-y-4">
          ...
          <div>
            ...
            <select value={form.unit} onChange={(e) => setForm((f) => ({ ...f, unit: e.target.value }))}
              ...
              {['pcs', 'kg', 'g', 'L', 'mL', 'box', 'bag', 'pair', 'set', 'dozen'].map((u) => (
          ...
          <div>
            ...
            {selectedCategories.length === 0 && (
              <p className="text-xs text-amber-500 mt-1">Please select at least one category</p>
            )}
          </div>
      ...
      <Modal open={!!deleteId} onClose={() => setDeleteId(null)} title="Delete Product?">
        ...
        <div className="flex justify-end gap-3">
          ...
          <motion.button whileTap={{ scale: 0.97 }} onClick={() => deleteMutation.mutate(deleteId)}
