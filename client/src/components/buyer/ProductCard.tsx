    ...
    <motion.div variants={cardVariants} whileHover={{ y: -4, transition: { duration: 0.2 } }}>
      <Link to={`/products/${product._id}`}
        ...
        <div className="aspect-square bg-gradient-to-br from-violet-50 to-indigo-50 overflow-hidden relative">
          ...
          {discountPct && (
            <span className="absolute top-2 left-2 bg-red-500 text-white text-xs font-black px-2 py-0.5 rounded-full">-{discountPct}%</span>
          )}
          {product.stock === 0 && (
            <div className="absolute inset-0 bg-white/70 flex items-center justify-center">
              <span className="text-xs font-bold text-slate-500 bg-white px-3 py-1 rounded-full border border-slate-200">Out of Stock</span>
            </div>
          )}
        </div>
        <div className="p-3">
          ...
          {product.vendor?.name && <p className="text-xs text-blue-600 font-medium mt-0.5 truncate">🏭 {product.vendor.name}</p>}
          <div className="flex items-center gap-2 mt-2">
            <span className="text-base font-black text-slate-900">{formatRs(product.price)}</span>
            {hasDiscount && <span className="text-xs text-slate-400 line-through">{formatRs(product.compareAtPrice!)}</span>}
          </div>
        </div>
      </Link>
    </motion.div>
