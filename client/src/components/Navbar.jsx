                      ...
                      {n.type === 'order' ? (
                        <>
                          ...
                          <div className="flex gap-2">
                            ...
                            <button onClick={() => window.open(`/invoice/${n.orderId}`, '_blank')}
                        ...
                        <>
                          ...
                          <p className="text-xs text-slate-600 bg-slate-50 rounded-lg px-3 py-2 mb-3 line-clamp-2">{n.subject}</p>
                          <button onClick={() => { navigate('/admin/messages'); onClose(); }}
                            className="w-full py-1.5 rounded-lg text-xs font-semibold bg-emerald-50 text-emerald-700 hover:bg-emerald-100 border border-emerald-200 transition-colors">
                            View Message
                          </button>
                        </>
                      )}
...
export default function Navbar() {
  ...
  const cartProductCount = cartData?.items?.length ?? 0;
        ...
        <div className="flex justify-between items-center h-16">
          ...
          <div className="hidden md:flex items-center gap-1">
            {navLinks.map(({ to, label, badge }) => (
              <Link key={to} to={to}
                className="relative px-3 py-2 rounded-lg text-sm font-medium text-slate-600 hover:text-violet-600 hover:bg-violet-50 transition-all">
                {label}
                {badge > 0 && (
                  <span className="absolute -top-0.5 -right-0.5 bg-violet-600 text-white text-xs font-black w-4 h-4 rounded-full flex items-center justify-center leading-none">
                    {badge > 9 ? '9+' : badge}
                  </span>
                )}
              </Link>
            ...
            <div className="px-4 py-3 space-y-1">
              {navLinks.map(({ to, label, badge }) => (
                <Link key={to} to={to} onClick={() => setMenuOpen(false)}
                  className="relative block px-3 py-2.5 rounded-lg text-sm font-medium text-slate-600 hover:text-violet-600 hover:bg-violet-50 transition-all">
                  {label}
                  {badge > 0 && (
                    <span className="ml-1.5 bg-violet-600 text-white text-xs font-black px-1.5 py-0.5 rounded-full">
                      {badge > 9 ? '9+' : badge}
                    </span>
                  )}
                </Link>
